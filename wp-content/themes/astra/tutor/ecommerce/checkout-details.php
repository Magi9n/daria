<?php
/**
 * Checkout info template - Custom Astra Theme Override
 *
 * @package Tutor\Views
...
<!-- Estilos CSS para el diseño personalizado del checkout -->
<style>
...
.checkout-course-link:hover {
    transform: translateX(-5px) !important;
    color: #592D36 !important;
}

.checkout-course-link::after {
    content: '' !important;
    position: absolute !important;
...
.checkout-course-link:hover::after {
    background-color: #592D36 !important;
}

.checkout-chevron-icon {
...
.checkout-course-link:hover .checkout-chevron-icon {
...
<div class="tutor-checkout-details">
	...
	<div class="custom-checkout-container">
		<div class="checkout-header">
			<span class="checkout-header-text">Detalles de tu Compra</span>
		</div>
		...
		if ( $plan_info ) {
			...
			if ( is_array( $course_list ) && count( $course_list ) ) :
				...
				foreach ( $checkout_data->items as $item ) :
					...
					?>
					<div class="checkout-item tutor-checkout-course-item" data-course-id="<?php echo esc_attr( $item->item_id ); ?>">
						<div class="checkout-product-info">
							<p class="checkout-product-name-text">
								Sé tu propia maquillista - 
								<span class="checkout-plan-type">
						...
						<a href="<?php echo esc_url( get_the_permalink( $course ) ); ?>" class="checkout-course-link">
							Ir al curso
							<span class="checkout-chevron-icon">›</span>
						</a>
						
						<!-- Datos ocultos necesarios para Tutor LMS -->
						<div class="hidden-checkout-data">
	...
	<!-- Mantener funcionalidad original de Tutor LMS oculta -->
		...
		<div class="tutor-checkout-details-inner">
			...
			<div class="tutor-checkout-detail-item">
				<div class="tutor-checkout-courses">
					<?php
					// Mantener la funcionalidad original pero oculta
					if ( $plan_info ) {
						$plan_item_template = apply_filters( 'tutor_checkout_plan_item_template', null, $plan_info );
						if ( file_exists( $plan_item_template ) ) {
							require $plan_item_template;
