 ...
 *
 * Copied from Tutor LMS and adjusted to include our custom checkout-details.php
 * from the theme override directory.
 */
			...
			<form method="post" id="tutor-checkout-form">
				...
				<div class="tutor-row tutor-g-5">
					<div class="tutor-col-md-6" tutor-checkout-details>
						<?php
						// Include the theme override version of checkout-details.php
					...
					<div class="tutor-col-md-6">
						<div class="tutor-checkout-billing">
							...
							<?php } ?>
								...
								<!-- Mantener el formulario y JS del plugin -->
								...
								<style>
								.billing-address-container { background: ##FFFFFF00; padding: 24px; border-radius: 8px; margin-bottom: 24px; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; }
								.billing-title { color: #8B7355; font-size: 20px; font-weight: 400; margin: 0 0 20px 0; letter-spacing: 0.5px; }
								.billing-row { display: flex; gap: 12px; margin-bottom: 12px; }
								.billing-field { flex: 1; margin-bottom: 12px; }
								.billing-input { width: 100%; padding: 12px 16px; border: 1px solid #d1d5db; border-radius: 6px; font-size: 14px; color: #6b7280; background: white; transition: all 0.2s ease; box-sizing: border-box; }
								.billing-input:focus { outline: none; border-color: #8B7355; box-shadow: 0 0 0 3px rgba(139, 115, 85, 0.1); }
								.billing-input::placeholder { color: #9ca3af; font-weight: 300; }
								@media (max-width: 768px) { .billing-row { flex-direction: column; gap: 0; } }
								</style>
							</div>
							<div class="tutor-payment-method-wrapper tutor-mt-20 <?php echo esc_attr( $show_payment_methods ? '' : 'tutor-d-none' ); ?>">
								<h5 class="tutor-fs-5 tutor-fw-medium tutor-color-black tutor-mb-12"><?php esc_html_e( 'Payment Method', 'tutor' ); ?></h5>
								<div class="tutor-checkout-payment-options tutor-mb-24">
									...
									<?php
									// Configuración de pasarelas (mantener la misma lógica original)
									...
									<?php if ( empty( $supported_gateways ) ) : ?>
										<div class="tutor-alert tutor-warning"><?php esc_html_e( 'No payment method found. Please contact the site administrator.', 'tutor' ); ?></div>
									<?php else : ?>
										<?php foreach ( $supported_gateways as $gateway ) : if ( ! is_array( $gateway ) ) continue; $name = $gateway['name'] ?? ''; $label = $gateway['label'] ?? $name; $icon = $gateway['icon'] ?? ''; $is_manual = $gateway['is_manual'] ?? false; ?>
										<label class="tutor-checkout-payment-item" data-payment-type="<?php echo $is_manual ? 'manual' : 'automate'; ?>">
											...
											<div class="tutor-payment-item-content"><?php echo $icon; echo esc_html( $label ); ?></div>
							...
							if ( $pay_now_errors || $pay_now_alert_msg ) : ?>
								<div class="tutor-break-word tutor-mt-16">
									<?php if ( ! empty( $pay_now_alert_msg ) ) : list( $alert, $message ) = array_values( $pay_now_alert_msg ); ?>
							...
							function processDirectPayment() {
								...
								var missingFields = [];
								requiredFields.forEach(function(field){ var input = document.querySelector('[name="'+field+'"]'); if(!input || !input.value.trim()){ missingFields.push(field); } });
								if (missingFields.length>0){ alert('Por favor completa todos los campos requeridos: '+missingFields.join(', ')); return; }
								var button=document.getElementById('tutor-checkout-pay-now-button'); button.disabled=true; button.innerHTML='Procesando...';
								if (paymentMethod.value==='woo-mercado-pago-basic'){ processMercadoPago(formData); }
								else if (paymentMethod.value==='ppcp-gateway'){ processPayPal(formData); }
								else { alert('Método de pago no soportado'); button.disabled=false; button.innerHTML='<?php echo esc_js( $pay_now_btn_text ); ?>'; }
							}
							function processMercadoPago(formData){
								var courseId=formData.get('object_ids');
								var billingData={ first_name:formData.get('billing_first_name'), last_name:formData.get('billing_last_name'), email:formData.get('billing_email'), phone:formData.get('billing_phone')||'', country:formData.get('billing_country'), address:formData.get('billing_address_1'), city:formData.get('billing_city'), state:formData.get('billing_state')||'', postcode:formData.get('billing_postcode')||'' };
								jQuery.ajax({ url:'<?php echo admin_url("admin-ajax.php"); ?>', type:'POST', data:{ action:'create_mercadopago_preference', course_id:courseId, billing_data:billingData, nonce:'<?php echo wp_create_nonce("mercadopago_nonce"); ?>' }, success:function(response){ if(response.success && response.data.init_point){ window.location.href=response.data.init_point; } else { alert('Error al procesar el pago con Mercado Pago'); resetButton(); } }, error:function(){ alert('Error de conexión con Mercado Pago'); resetButton(); } });
							}
							function processPayPal(formData){
								var courseId=formData.get('object_ids');
								var billingData={ first_name:formData.get('billing_first_name'), last_name:formData.get('billing_last_name'), email:formData.get('billing_email'), phone:formData.get('billing_phone')||'', country:formData.get('billing_country'), address:formData.get('billing_address_1'), city:formData.get('billing_city'), state:formData.get('billing_state')||'', postcode:formData.get('billing_postcode')||'' };
								jQuery.ajax({ url:'<?php echo admin_url("admin-ajax.php"); ?>', type:'POST', data:{ action:'create_paypal_order', course_id:courseId, billing_data:billingData, nonce:'<?php echo wp_create_nonce("paypal_nonce"); ?>' }, success:function(response){ if(response.success && response.data.approval_url){ window.location.href=response.data.approval_url; } else { alert('Error al procesar el pago con PayPal'); resetButton(); } }, error:function(){ alert('Error de conexión con PayPal'); resetButton(); } });
							}
							function resetButton(){ var button=document.getElementById('tutor-checkout-pay-now-button'); button.disabled=false; button.innerHTML='<?php echo esc_js( $pay_now_btn_text ); ?>'; }
							</script>
						</div>
					</div>
				</div>
...
<?php if ( ! is_user_logged_in() ) { tutor_load_template_from_custom_path( tutor()->path . '/views/modal/login.php' ); } ?>
